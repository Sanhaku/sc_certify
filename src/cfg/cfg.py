...
class CFG(object):
    ...
    def data_dependence(self, ins):
        if not ins in self._dd:
            from src.cfg.slicing import backward_slice
            self._dd[ins] = set(i for s in backward_slice(ins)
                                for i in s if i.bb)
        return self._dd[ins]
    ...
    def _compute_dominators(self):
        ...
        self._dominators = {self._bb_at[k]: self._bb_at[v]
    ...
    def to_dot(self, minimal=False):
        ...
        for bb in sorted(self.bbs):
            ...
            from_block += 'From: ' + \
                ', '.join('%x' % pred.start for pred in sorted(bb.pred))
            ...
            to_block = 'To: ' + \
                ', '.join('%x' % succ.start for succ in sorted(bb.succ))
    ...
    def from_json(json_dict, code):
        from src.cfg.disassembly import disass
        bbs = list()
    ...
    def find_loops(self):
        ...
        for i in l:
            ...
            head = [succ.start for bb in loop_bbs for succ in bb.succ if succ.start in i if bb.start > succ.start and len([p.start for p in succ.pred if p.start not in i]) != 0 and len(
                succ.succ) <= 2 and ('ADD' in [ins.name for ins in bb.ins] or [ins.name for ins in bb.ins[-2:]] == ['PUSH2', 'JUMP'])]
            ...
            back_edge = [[succ.start, bb.start]
                         for bb in loop_bbs for succ in bb.succ if succ.start in i if bb.start > succ.start]
            loop.back_edges = back_edge
