...
from src.rattle import SSAInstruction
from src.sym_exec.state import State
from src.sym_exec.utils import get_argument_value, is_concrete, WORD_SIZE, is_symbolic
...
def inst_sload(instruction: SSAInstruction, state: State):
    ...
    if value is None:
        if is_symbolic(key):
            bv_name = f"storage_sym_{args[0].value}"
        else:
