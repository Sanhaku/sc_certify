...
def run_trace_has_loop_body(trace,loop):
    ...
    for bb in trace[(len(trace)-len(loop)):]:
        dynamic_gas = 0
        for ins in bb.ins:
            opcode = ins.op
            op = ins.name
...
def run_trace(trace, loops: List[Loop]):
    ctx = Context()
    aset = AccessSets()
    stk = Stack()
    mem = Memory()
    ...
    is_has_loop = has_loop(trace,loops)

    for bb in trace:
        # static_gas += bb.static_gas
        dynamic_gas = 0
        for ins in bb.ins:
            opcode = ins.op
            op = ins.name
            ...
            # SELFDESTRUCT opcode (also called SELFDESTRUCT)
                ...
                raise ExternalData('SELFDESTRUCT')
        if bb_is_exist_in_loop(loops,bb.start):
            loop_gas += bb.static_gas + dynamic_gas
        else:
            base_gas += bb.static_gas + dynamic_gas
    return base_gas,dynamic_gas


# def run(key_traces: DefaultDict[int, List], loops: List[Loop], function: int):
#     function_traces = key_traces[int]
    
#     mini_trace = defaultdict(list)
#     mini_gas = 3000000
